...
| 34 | **Forgot password / OTP reset** *(new — `[[forgot-password-otp-reset]]`, Draft)* | `users` migration adding `reset_otp_hash`/`reset_otp_expires_at`/`reset_otp_attempts`/`reset_otp_sent_at`; `POST /api/auth/forgot-password` + `POST /api/auth/verify-otp`; `sendEmail()` primitive extracted from `notification-provider.ts`'s Gmail/Resend/mock selection; new `frontend/portal` `/forgot-password` page + a "Forgot password?" link on `/login` | #21 (auth — extends the `users` table and login/signup UI it introduced) | Medium — two new endpoints, a DB migration, an email-transport refactor shared with `notify-claimant`, and a genuinely open design question (auto-generated vs. user-chosen new password) still to resolve before Lock |
