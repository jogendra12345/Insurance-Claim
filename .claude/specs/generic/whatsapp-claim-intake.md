...
4. ~~**Claimant identity / authorized-claimant check**~~ — **Resolved 2026-09-16, implemented ahead of the rest of this spec:** `channel = 'whatsapp'` claims are authorized on **phone number only** (`claimant_phone` against `policies.policyholder_phone`/`policy_dependents.phone`), not email/name — the opposite of this question's original lean, decided because the phone number is the one piece of identity a WhatsApp conversation itself verifies (it's who's actually messaging), whereas email/name typed in-chat would carry no more assurance than a portal submission's. See `.claude/specs/worker/validate-claim.md`'s 2026-09-16 addendum and `SPEC.md` §9 "Channel-scoped signal" for the full design and the new `policyholder_phone`/`policy_dependents.phone`/`claims.claimant_phone`/`claims.channel` columns (migration `0015_add_phone_fields.sql`). **Still open:** this webhook itself doesn't exist yet, so nothing sets `channel = 'whatsapp'` or `claimant_phone` in production today — building the actual webhook (per whichever Open Question 1 answer is chosen) is what would populate them from the sending phone number and finally exercise this path.
