...
- Query: `SELECT * FROM audit_log WHERE claim_id = $1 [AND actor_type = $n] [AND created_at >= $n] [AND created_at <= $n] ORDER BY created_at DESC` — conditions appended the same parameterized-query pattern `GET /api/claims` already uses for its optional `policyNumber` filter. Newest first, per the 2026-09-15 revision note.
