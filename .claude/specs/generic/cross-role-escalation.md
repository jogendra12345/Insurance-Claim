...
- Notifying the legal team differently than a normal DMN-routed legal claim would — `[[reviewer_task_notification_test_mode]]`'s existing `notifyRole('legal-reviewers', ...)` call (wherever it's wired into the legal-review entry point) covers this without changes, since escalation and DMN-routing both land on the same `Legal Review` task.
...
4. **Interaction with `[[reviewer_task_notification_test_mode]]`:** confirm that whichever job-worker point currently triggers the "Legal Review task is waiting" notification is keyed off the task actually opening (which `capture-escalation`'s loop-back would still trigger), not off `capture-routing-decision`/DMN output specifically — otherwise an escalated claim's legal reviewers would never get notified. Needs a check against that spec's actual wiring before this ships.
