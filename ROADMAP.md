...
- [ ] **6. Human review** — Tasklist candidate groups configured, triage → role review → sign-off walked through manually. No longer blocked (see "Open decision before Step 6" below — resolved via Option B / app-level auth, `.claude/specs/generic/auth-role-based-access.md`, built 2026-09-04). Triage Review verified live through the new `/tasks` page as a real distinct `triage-team` user (not `demo`) — role → BPMN candidate group mapping, task claim/complete proxy, and downstream routing all confirmed working end to end. Adjuster/Investigator/Legal Review and Supervisor Sign-off still need the same walkthrough before checking this off.
...
**Decided 2026-09-04: Option 3 (Option B / app-level auth).** Locked as `.claude/specs/generic/auth-role-based-access.md` and built the same day — `users` table (migration `0010_add_users.sql`), claimant self-registration gated by a policy-number + email match, staff accounts seeded directly (no admin UI in this pass), signed-cookie sessions, `GET/POST /api/tasks*` proxying Tasklist with the app's role → candidate-group map enforced server-side, and a staff-only `/tasks` page in `frontend/portal`. Staff test accounts today: `triage1@claimflow.test` / `adjuster1@claimflow.test` (password `adjuster123`, seeded via raw SQL — see the spec's lock note). This pulls `BUILD-PLAN.md` Phase 2 item #21 forward; the `moreInfo` resubmit endpoint and audit view (§14) remain unbuilt but are now unblocked.
