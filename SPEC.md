...
| `notify-claimant` | `claimId`, `decision` | Reads `claimant_name`, `claimant_email`, and (on `deny`) `denial_letter_text` from `claims`; calls `NotificationProvider.send()` — real send via Gmail SMTP when `GMAIL_USER`/`GMAIL_APP_PASSWORD` are set (preferred: delivers to any claimant address), else via Resend when `RESEND_API_KEY` is set (sandbox-restricted to the account's own address until a domain is verified), else falls back to the mock (logs instead of sending) | `notificationSent` (bool) |
...
- **Loop `moreInfo` back into the process instead of ending it.** Today the `Decision` gateway's `moreInfo` branch is a terminal end event ("Awaiting More Information," §10 step 14) — `claims.status` becomes `awaiting_info` and the claim goes nowhere. The planned mechanism:
  - Replace that end event with a **BPMN message event** the process instance waits on, correlated by `claimId`.
  - Re-entry point is **back to the reviewing role's User Task** (whichever of `Adjuster Review`/`Investigator Review`/`Legal Review` requested the info), not back to intake — this avoids re-running AI extraction/scoring unnecessarily and matches what a real reviewer asking for more documents would expect.
  - A new `claims.info_requested_reason` column (or equivalent), written by `capture-review-decision`'s `moreInfo` branch, captures *why* more info was requested — nothing today records this (`denial_reason` is deny-only).
  - New `POST /api/claims/:id/resubmit` endpoint: claimant adds documents (and optionally a note) via the same upload widget as the original submission form, inserts new `claim_documents` rows, writes `audit_log`, then publishes the correlation message via `@camunda8/sdk` to resume the *same* process instance (same `processInstanceKey`, same claim row — not a new claim or a new instance).
  - Frontend: the claimant status page detects `status = 'awaiting_info'` and swaps its normal read-only view for a resubmission form (showing `info_requested_reason`, file upload, optional note) that posts to the endpoint above; once the process resumes past that point, the page reverts to read-only status display.
  - **Two email touchpoints via the existing `NotificationProvider`** (extends `notify-claimant`, §12): (1) when `moreInfo` is decided, the `Decision` gateway's `moreInfo` branch calls `notify-claimant` with a new `decision = "moreInfo"` case — email includes `info_requested_reason` and a link to the status page's resubmission form, same pattern `buildEmail()` already uses for approve/deny; (2) once `POST /api/claims/:id/resubmit` succeeds and the message is published, a second `notify-claimant`-style call confirms the resubmission was received and the claim is back under review — otherwise the claimant has no confirmation their upload actually resumed the process.
  - **Depends on auth** (the "Auth + role-based access" item below) — without it, anyone with a claim ID could inject documents into someone else's claim via the resubmit endpoint.
