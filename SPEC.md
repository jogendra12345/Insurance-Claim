...
policies
  ...
  policyholder_phone text NULL            -- authorized-claimant check for WhatsApp-channel claims only (below); optional, portal-channel claims never read it
...
policy_dependents
  ...
  email        text NOT NULL
  phone        text NULL     -- authorized-claimant check for WhatsApp-channel claims only (below); optional
...
claims
  ...
  claimant_phone        text NULL            -- only read by the authorized-claimant check when channel = 'whatsapp' (below); portal claims may leave it blank
  channel               text NOT NULL DEFAULT 'portal'  -- portal | whatsapp — decides which authorized-claimant signal validate-claim trusts (below); 'whatsapp' unused until .claude/specs/generic/whatsapp-claim-intake.md ships
  ...
  assigned_role         text NULL            -- adjuster | investigator | legal (DMN suggestion); written by capture-routing-decision
  confirmed_role        text NULL            -- role a human actually routed it to, after triage review; written by capture-triage-review
  triage_note           text NULL            -- optional handoff note from the triage reviewer to the adjuster/investigator/legal reviewer; written by capture-triage-review on the "review" outcome only
  ...
  info_requested_reason text NULL            -- written by capture-review-decision's moreInfo branch; mirrors denial_reason's shape, never cleared on a later status change
...
**Channel-scoped signal (2026-09-16):** the email/name check above only applies when `claims.channel = 'portal'` (today's only real value). A `channel = 'whatsapp'` claim is authorized on **phone number only** — `claimant_phone` must match `policies.policyholder_phone` or a `policy_dependents.phone` row for the matched policy, exactly, with no email/name fallback. Rationale: a WhatsApp conversation's phone number is the one piece of the claimant's identity that channel itself verifies (it's who's actually messaging), whereas name/email on that channel would just be self-typed text with no more assurance than a portal submission's — so phone is the stronger, channel-appropriate signal there, while portal submissions (which have no verified phone) keep the original email/name check unchanged. `policyholder_phone`/`policy_dependents.phone` are both optional (`NULL`-able, no backfill) — a policy with no phone on file simply can never pass the WhatsApp-channel check, which is correct today since `channel = 'whatsapp'` is unused until `.claude/specs/generic/whatsapp-claim-intake.md` ships (this addendum resolves that spec's Open Question 4). `POST /api/policies` accepts an optional `policyholderPhone` and per-dependent `phone` alongside the existing fields; the "Add policy" panel collects both.
...
2. **Service Task** `validate-claim` — required fields + policy status, using the config for this claim's `insuranceType`; looks up `policies` by `policyNumber` and `carrierId`; if matched, checks for a duplicate — another claim already on that `policy_id` with `status NOT IN ('approved', 'denied')` — and fails validation if one exists (one in-flight claim per policy at a time); also runs the channel-scoped authorized-claimant check — `channel = 'portal'` (default) checks `claimant_email` **or** `claimant_name` (case-insensitive, either is enough) against `policyholder_email`/`policyholder_name` and `policy_dependents.email`/`full_name`; `channel = 'whatsapp'` checks `claimant_phone` against `policyholder_phone`/`policy_dependents.phone` instead, no email/name fallback — for the matched policy, failing validation if the applicable signal matches no row (see "Authorized claimants" under §9) — sets `claims.policy_id` on match, sets `claims.status = 'validating'` only when all three checks pass, and computes two deterministic red-flag signals for the DMN table below: `daysSincePolicyEffective` and `claimantClaimCountLast12Months`
3. **Exclusive Gateway** `Validation Passed?`
   - No → **User Task** `Validation Exception Review` (candidate group `triage-team`) — a `resolutionAction` field lets the reviewer either approve (continue) or reject outright → **Service Task** `capture-validation-exception` → **Exclusive Gateway** `Validation Exception Decision?`: `resolve` → step 4 (`extract-evidence`); `reject` (default) → the denial path (step 16, same as `Gateway_TriageDecision`'s reject branch). **`resolve` only continues the claim as originally submitted** — there is deliberately no way to edit the policy number from this form. Available for *any* validation failure, including "no policy matched at all" — the claim continues with `claims.policy_id` staying `NULL` if nothing matched. This is a deliberate human-override: a triage reviewer may have context the automated check doesn't (e.g. a coverage-window edge case, a policy update not yet reflected in `policies`), and per §1/§15 a human is always allowed to override an automated outcome. The tradeoff, stated plainly: an approved-with-`policy_id = NULL` claim skips every downstream check that depends on a matched policy — there is no re-validation of coverage amount, active status, or the incident-date window once this path is taken, so the reviewer is the sole check. (An earlier version of this spec disallowed resolving with no policy match and had `capture-validation-exception` throw into an Operate incident instead — reverted per explicit product direction; the earlier reasoning is preserved here since the tradeoff it described is still real, just now accepted rather than blocked.)
   - Yes (default) → step 4
...
7. **Business Rule Task** — evaluates the DMN decision selected via `decisionIdExpression` for this claim's `insuranceType` (v1: `health-claim-routing-decision`) → sets `assignedRole` (`adjuster` | `investigator` | `legal` — a low-risk/low-value claim that matches none of the fraud/risk/amount/frequency rules still gets a real suggested role, `adjuster`, as the catch-all; there is no `"auto"` pseudo-role)
...
9. **User Task** `Triage Review` (candidate group `triage-team`, form `TriageReviewForm` — `process/forms/triage-review.form`) — reviewer sees the AI's case summary, fraud indicators, risk score, and suggested role; the form's `triageAction` field (required) is either:
   - `"review"` (default outcome) — sets `confirmedRole` (may differ from `assignedRole`; field required by the form when this action is chosen) and an optional `triageNote` — a free-text handoff note the role-specific reviewer (adjuster/investigator/legal) sees on their own review task, shown via `{{triageNote}}` on `ReviewDecisionForm`
   ...
   **Every claim passes through this task — `assignedRole` is always one of the three real roles (never a skip), and the reviewer accepts or overrides it via `confirmedRole`** (see §1, §15; this replaced an earlier "auto-bypass" gateway that let low-risk claims skip human review entirely, which contradicted §1's "human confirming every AI-driven routing decision" — the DMN's old `"auto"` catch-all output was later dropped too, in favor of always suggesting `adjuster` for a low-risk/low-value claim, so the AI never suggests a non-role and the reviewer is always confirming a real one). `triageAction`/`confirmedRole`/`denialReason` are required directly by the form — Tasklist's default (no-form) "Complete" button previously let a reviewer complete this task with zero variables, silently losing the confirmed role; the form is what actually enforces the "human confirms" guarantee, not just the BPMN shape.
10. **Service Task** `capture-triage-review` — branches on `triageAction`:
    - `"review"` → writes `claims.confirmed_role = confirmedRole` and `claims.triage_note = triageNote` (NULL if not provided), sets `claims.status = 'in_review'`, writes `audit_log` (action `triage_confirmed`, flags an override when `confirmedRole != assignedRole`, includes `triageNote` in `detail` when present)
...
17. More-info path (`.claude/specs/generic/claimant-more-info-resubmission.md`, Locked 2026-09-11 — replaces the earlier terminal end event): **Service Task** `notify-claimant` (`decision = "moreInfo"`, emails the claimant `info_requested_reason` + a link to the claim) → **User Task** `Claimant Provide More Info` (**no candidate group** — unlike every staff task, this one is never listed via `GET /api/tasks`; the claimant portal locates it directly by `processInstanceKey` + element ID via `GET /api/claims/:id/pending-task`, and completes it via `POST /api/claims/:id/resubmit`, both claimant-scoped and ownership-checked, separate from the staff task proxy) → **Service Task** `capture-claimant-resubmission` (sets `claims.status = 'in_review'`, writes `audit_log`, best-effort `notifyRole()` to the reopened role) → **Exclusive Gateway** `Route Back by Confirmed Role` (same four-way split as `confirmedRole`, default `adjuster`) → back into **whichever of** `Adjuster Review` / `Investigator Review` / `Legal Review` / `Supervisor Review` **originally asked** — not back to intake, so `extract-evidence`/`detect-fraud-indicators`/`score-risk` don't rerun. That reviewer's next decision re-enters step 13 exactly as any other role-review completion does; a claim can loop through this path any number of times (no cap, no SLA/timeout on the claimant's own task — see §14).
...
| – | ≥ 40 | – | – | – | adjuster |
| – | – | > 5000 | – | – | adjuster |
| – | – | – | – | – | adjuster *(catch-all — no fraud/risk/amount/frequency rule matched; still a real suggested role, not `"auto"`)* |
...
|---|---|---|---|
| `validate-claim` | `insuranceType`, `carrierId`, `policyNumber`, `claimAmount`, `incidentDate` | Required-field check using the insurance-type config (§3) for which fields are required; looks up `policies` by `policyNumber` + `carrierId` and checks `status = 'active'` and `incidentDate` within `[effective_date, expiry_date]`; on a match, also checks for a duplicate in-flight claim on the same `policy_id` (`status NOT IN ('approved', 'denied')`) and fails validation if one exists; also runs the channel-scoped authorized-claimant check on the matched policy — `channel = 'portal'` checks `claims.claimant_email` **or** `claimant_name` (case-insensitive, either is enough) against `policyholder_email`/`policyholder_name` and `policy_dependents.email`/`full_name`; `channel = 'whatsapp'` checks `claimant_phone` against `policyholder_phone`/`policy_dependents.phone` instead — failing validation if the applicable signal matches no row (§9 "Authorized claimants"); sets `claims.policy_id` and (on pass of all checks) `claims.status = 'validating'`; computes `daysSincePolicyEffective` and the claimant's trailing-12-month claim count for the DMN table (§11) | `validationPassed` (bool), `policyId` (uuid, nullable), `duplicatePendingClaim` (bool), `duplicateClaimId` (uuid, nullable), `authorizedClaimant` (bool), `daysSincePolicyEffective` (number, nullable), `claimantClaimCountLast12Months` (number) |
...
| `capture-triage-review` | `claimId`, `triageAction`, `confirmedRole` (when reviewing), `assignedRole`, `denialReason` (when rejecting), `triageNote` (optional, when reviewing) | Branches on `triageAction`: `"review"` writes `claims.confirmed_role` + `claims.triage_note`, sets `status = 'in_review'` (`audit_log.detail` flags an override); `"reject"` writes `claims.decision = 'deny'` + `denial_reason`, sets `status = 'denied'` (same shape as `capture-review-decision`'s deny branch) | — |
| `capture-review-decision` | `claimId`, `decision`, `denialReason`, `infoRequestedReason`, `confirmedRole` | Writes `claims.decision`, `claims.denial_reason`, `claims.info_requested_reason`; sets `claims.status` from `decision` (`approve→approved`, `deny→denied`, `moreInfo→awaiting_info`) | — |
...
| `capture-claimant-resubmission` | `claimId`, `confirmedRole`, `resubmittedByUserId`, `documentCount` | `.claude/specs/generic/claimant-more-info-resubmission.md` (§10 step 17) — sets `claims.status = 'in_review'`; writes `audit_log` (`actor_type: "human"`, `actor_id` = the claimant's `users.id`, action `claimant_resubmitted`); best-effort `notifyRole()` to whichever role is reopening (mirrors `auto-escalate-review`'s notification, but doesn't set an SLA deadline — no timeout on this loop) | — |
...
| `notify-claimant` | `claimId`, `decision` | Reads `claimant_name`, `claimant_email`, and (on `deny`) `denial_letter_text` or (on `moreInfo`) `info_requested_reason` from `claims`; calls `NotificationProvider.send()` — real send via Gmail SMTP when `GMAIL_USER`/`GMAIL_APP_PASSWORD` are set (preferred: delivers to any claimant address), else via Resend when `RESEND_API_KEY` is set (sandbox-restricted to the account's own address until a domain is verified), else falls back to the mock (logs instead of sending). Runs on the approved, denied, **and** more-info paths (three call sites — `decision` distinguishes the email body) | `notificationSent` (bool) |
...
`SettlementProvider` and `NotificationProvider` are TypeScript interfaces; `SettlementProvider` still has only a mock implementation in v1. `NotificationProvider` gained a real implementation (Resend, `backend/shared/notification-provider.ts`) per explicit product direction on 2026-08-31 — `notify-claimant` picks it over the mock whenever `RESEND_API_KEY` is set (PREREQUISITES.md). Resend's free-tier sandbox sender only delivers to the address the Resend account was signed up with, so real delivery to arbitrary claimant addresses still needs a verified domain. A second real implementation (Gmail SMTP via Nodemailer, same file) was added 2026-09-02 and is preferred over Resend whenever `GMAIL_USER`/`GMAIL_APP_PASSWORD` are set — it relays through a real Gmail mailbox, so it delivers to any claimant address today with no domain verification, at the cost of lower sending limits and mail arriving from a personal address rather than a dedicated sender. **Test-mode note (2026-09-09):** per the same product direction as the reviewer-notification emails (`.claude/specs/generic/reviewer-task-notification-emails.md`'s lock note), since this is a small internal test app (`[[project_demo_app_no_real_payments]]`) and seeded claimant accounts use undeliverable `@example.com` addresses, both `resendNotificationProvider` and `gmailNotificationProvider` in `backend/shared/notification-provider.ts` redirect the actual send's `to` to a single fixed `TEST_RECIPIENT = "ayanchou2015@gmail.com"` instead of `claims.claimant_email`, tagging the real claimant email into the subject line instead (`[claimant@email] <subject>`) so it's still visible in the inbox. This is a deliberate, temporary simplification — swap `TEST_RECIPIENT` back out (two call sites, one file) for `context.claimantEmail` whenever this moves past test/demo use.
...
- A cap on how many times a single claim can loop through the `moreInfo` path (§10 step 17), and any SLA/timeout on the claimant's own `Claimant Provide More Info` task — a claimant who never responds leaves their claim in `awaiting_info` indefinitely (locked decision, `.claude/specs/generic/claimant-more-info-resubmission.md`, 2026-09-11). `.claude/specs/generic/sla-review-escalation.md`'s SLA timers only cover reviewer-side tasks.
- Editing original claim fields (amount, incident description, etc.) during a `moreInfo` resubmission — only new documents and an optional note, same shape as the original submission's upload widget.
- Re-running `extract-evidence`/`detect-fraud-indicators`/`score-risk` against documents added during a `moreInfo` resubmission — intentionally skipped; see §10 step 17.
...
- **Auth + role-based access for the frontend portal, plus a custom in-app task page.** Today there's no authentication anywhere — `GET /api/claims`/`/api/policies` return every row to anyone, and Camunda's lightweight stack has exactly one identity (`demo`), so every Tasklist action is performed as `demo` regardless of which BPMN candidate group a task is actually restricted to. Recommended shape (Option B, chosen over switching to `docker-compose-full.yaml`'s real Camunda-native Identity/Keycloak — sidesteps the still-open decision in ROADMAP.md "Open decision before Step 6" rather than requiring it):
  - A `users` table (`email`, `password_hash`, `role` — `claimant | admin | triage-team | adjuster | investigator | legal-reviewer | supervisor`). Claimants self-register, defaulting to `role = 'claimant'`, no role picker exposed. Every staff role is admin-provisioned only, never self-selectable — letting someone pick "I'm a supervisor" at signup would let anyone grant themselves settlement-approval authority.
  - `GET /api/claims`/`/api/policies` scoped to `claimant_email = current user` for `role = 'claimant'` (same email-matching approach `validate-claim`'s authorized-claimant check already uses, §9); unscoped for `role = 'admin'`.
  - An explicit role → BPMN-candidate-group map (not a string transform, since names aren't identical): `triage-team→triage-team`, `adjuster→adjusters`, `investigator→investigators`, `legal-reviewer→legal-reviewers`, `supervisor→supervisors`, `admin→`(all groups), `claimant→`(none — never sees the task page).
  - The app's own auth is the trust boundary — Camunda stays behind it as a trusted backend service. `backend/api` holds the single `demo` credential server-side and adds `GET /api/tasks` (maps the caller's role to a candidate group, calls Tasklist's `POST /v2/user-tasks/search`, joins results against `claims` for context), `POST /api/tasks/:key/claim`, and `POST /api/tasks/:key/complete` (proxy `/v2/user-tasks/:key/assignment`/`/completion`) — end users never receive or use Camunda credentials.
  - A new frontend route (e.g. `/tasks`, staff-only) lists and completes tasks via the endpoints above, reusing the same field sets as today's Camunda forms (`TriageReviewForm`, `ReviewDecisionForm`, `ValidationExceptionReviewForm`) so behavior stays identical, just inside this app's own UI instead of Camunda's stock Tasklist (which keeps working standalone at `localhost:8080/tasklist` for direct debugging).
  - Open question worth flagging now: whether Camunda's lightweight (no-Identity) mode accepts an arbitrary `assignee` string on `/v2/user-tasks/:key/assignment`. If yes, the real app-user's email could be passed through so Camunda's own audit trail/Operate history stays accurate; if no, "who really acted" only lives in this app's own `audit_log`, and Camunda's `assignee` will always show `demo` — a cosmetic gap, not a security one, since access control already happens before the proxy call.
- Customer-facing status page pulling live process state.
- **Audit view — an in-app page showing a claim's full `audit_log` history as a readable timeline.** Staff-only (reuses the auth from the item above); given a `claim_id`, renders every `audit_log` row in chronological order (actor type/id, action, timestamp, details), the same merge `/case-trace` already does ad hoc against Camunda process history, but as a permanent UI page instead of a one-off skill invocation. Backed by a new `GET /api/claims/:id/audit` endpoint (reads `audit_log` filtered by `claim_id`, no new tables). Depends on the auth item above for access control — an unauthenticated audit view would leak case history (including AI risk/fraud findings) to anyone with a claim ID.
...
- **Direct link to user-submitted form.** Reviewer task details (`/tasks/:key`) and claim views (`/claims/:id`) gain an accessible reference link back to the claimant's originally submitted form/data, not just the extracted summary — lets a reviewer or staff member jump straight to what was actually filed.
- **Selectable document viewer.** Today's inline document preview (`/tasks/:key`, image/PDF rendering per `.claude/specs/generic/claimant-more-info-resubmission.md` and the original claim-detail work) doesn't support interactive text selection/copy. Needs a viewer capable of it for text-bearing documents (PDFs especially).
- **Audit log update for latest action.** Confirm/enforce that whatever "current state" a claim or task view surfaces reflects strictly the most recent `audit_log` action for that claim — not a stale or out-of-order read. Likely a read-side fix (query ordering/caching) rather than a new write path, since every write already goes through `writeAuditLog` (§13).
- **Time zone standardization.** Pick and document the one standard time zone this system logs and displays timestamps in (today `created_at`/`updated_at` are plain `timestamptz`, but nothing pins what zone the UI renders them in, e.g. `toLocaleString()` currently uses the viewer's local browser zone) — needed before timestamps across staff in different zones (or the audit trail view, `.claude/specs/generic/staff-audit-trail-view.md`) can be compared consistently.
- **Email & WhatsApp claim intake.** A multi-channel ingestion path letting a claimant initiate a claim via email or WhatsApp instead of only the portal's `POST /api/claims` form — likely an inbound webhook/parser per channel that maps to the same claim-submission shape, ahead of Camunda process kickoff.
- **Chatbot integration.** Evaluate and, if adopted, build an automated conversational assistant for claim filing — a guided/conversational front-end to claim intake, distinct from (and likely feeding into) the same submission path as the portal form.
- **Policy creation enhancements.** Upgrade the policy creation workflow (today's `/policies` "Add policy" panel, admin-only) for more flexibility/efficiency — exact shape TBD (e.g. bulk creation, richer dependent management at creation time, validation improvements).
- **Dynamic form for claim creation.** Evaluate and implement adaptive claim-submission form fields that change based on claim type and other user input, rather than today's fixed field set — likely ties into the §3 insurance-type extension pattern (different insurance types already imply different required fields/config modules) but may also apply within a single type (e.g. `claimType` sub-variants).

## 15. Definition of done (v1)

A health insurance claim can be submitted through the frontend, flows through the full BPMN process with real Gemini API calls at every AI step, a low-risk/low-value claim is still confirmed by a triage reviewer even though the DMN's catch-all suggests the lightest-weight real role (`adjuster`) rather than skipping review (the triage reviewer can accept or override that suggestion, never skip it — see §10), a claim with fraud indicators routes to an investigator, a high-value claim routes to legal, a triage reviewer can override the AI's suggested role or reject an obviously invalid/fraudulent claim outright without a full role-specific review, a large approved settlement requires supervisor sign-off, and every step — automated or human — leaves a row in `audit_log` that reconstructs the full case history from intake to resolution. The process, workers, and DMN selection are all insurance-type-parameterized even though only `health` ships in v1.
